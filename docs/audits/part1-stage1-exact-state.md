...
The second private CloudShell bundle was observed at **2026-09-24 17:45:14 UTC** in the same account, now in the configured `ap-south-2` region (ZIP SHA-256 `1711fbc41d8f13d65a97a9061a9c88d04ba75969663ddaba99d602f3e8ae7b69`). Its 26-file manifest verified, all eight read commands exited 0 with empty stderr, and caller, role ID, role trust, tags and absent role policy listings matched the first bundle. The user also supplied the separate `aws account get-region-opt-status --region-name ap-south-2` output reporting `ENABLED`; that console output is not part of either ZIP.

In **both** `ap-south-1` and `ap-south-2`, the Tagging API `Project=EventPulse` query returned no resource mappings, the SQS `eventpulse-` prefix query returned no queues, and the CloudWatch Logs `/aws/eventpulse/` prefix query returned no groups. These are **absence within those successful query scopes**, not a complete account inventory. Untagged or differently named Kinesis streams, DynamoDB tables, S3 buckets, queues and log groups have not been excluded. Budget and cost remain `UNKNOWN`. The identity, trust, account and configured-region boundary is verified. Draft PR #1's Terraform default divergence is a future deployment correction: explicitly set the region to `ap-south-2` and validate before any plan/apply. This audit does not change that draft or assert a deployment occurred.
...
| EP-S1-F07 | Fresh read-only evidence verified EventPulse OIDC trust in account `887720497919` and three empty query scopes in each of `ap-south-1` and `ap-south-2`; GitHub selects `ap-south-2`, while draft Terraform defaults to `ap-south-1`. | The configured boundary is established. Carry the Terraform default correction into the draft's eventual review before deployment. Resource absence and EventPulse-specific cost remain unproven. |
